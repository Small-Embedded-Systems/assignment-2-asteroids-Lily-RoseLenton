...
static void shipMovement() {
	...
	if (getTotalSpeed() < 2)
	{
		...
		//If the player is accelerating increase velocity in the direction of the heading
			...
			player.velocity.x += player.engines * xperc * 0.0005f;
			player.velocity.y -= player.engines * yperc * 0.0005f;
		}
	}
	drag();
...
void missileValues(struct missile *m){
	m->velocity.x = directionx(player.heading) * 0.01f;
	m->velocity.y = -directiony(player.heading) * 0.01f;
...
/* Update the asteroids positions using their velocity and remove an asteroid if it has been destroyed */
...
/* Update the missiles positions using their velocity and remove an missile if it has been destroyed */
...
/* If an asteroid has passed the boundaries of the screen move it to the other side of the screen */
